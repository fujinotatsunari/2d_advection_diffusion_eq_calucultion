...
string directories_setup(int n) {
	...
	string str;//makedirectoriesで帰ってくるパス
	string str1;//strのパスに追加したいディレクトリ
...
void output_csv(int n, double time, int xelem, int yelem, double* x, double* y, double* phi) {
	...
	std::string str;
	std::string str1;//makedirectoriesで帰ってくるパス
	std::string str2;
	std::string str3;
...
void output_inp(int n, double time, int xelement, int yelement, double* x, double* y, double* phi) {
	...
	int** nbool1;   //nbool[要素番号][局所節点番号] = 全体節点番号
	...
	for (int i = 0; i < nnode; i++) {       //節点番号 x,y,z座標
		//2次元よりz=0
		outputfile << i << " " << x[i] << " " << y[i] << " " << 0.0 << "\n";

	}
	for (int ie = 0; ie < nelem; ie++) {        //要素番号　材質番号　要素種類　要素構成
	...
	for (int i = 0; i < nnode; i++) {   //節点番号　節点データ値
		outputfile << i << " " << phi[i] << "\n";
	}
	outputfile.close();
